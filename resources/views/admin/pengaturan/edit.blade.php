        ...
        <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-gray-200 border-0">
            <div class="rounded-t bg-white mb-0 px-6 py-6">
                <div class="text-center flex justify-between">
                    <h6 class="text-gray-800 text-xl font-bold">Ubah Pengaturan</h6>
                    <div>
                        <a href="{{ url('/admin/pengaturan') }}" title="Back">
                            <button
            ...
            <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
                <form action="{{ url('/admin/pengaturan/' . $pengaturan->id) }}" method="post" accept-charset="UTF-8"
                      enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <h6 class="text-gray-500 text-sm mt-3 mb-6 font-bold uppercase"> Pengaturan </h6>
                    @include ('admin.pengaturan.form', ['formMode' => 'edit'])
                </form>
            </div>
        </div>
    </div>
