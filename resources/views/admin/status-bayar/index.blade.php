<x-app-layout>
    ...
    @livewire('admin.components.navbar',['active'=>'Manajemen StatusBayar'])
                ...
                <div class="rounded-t mb-0 px-4 py-3 border-0">
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full pr-4 max-w-full flex-grow flex-1 flex items-center">
                            <h3 class="font-semibold text-lg text-gray-800"> Data StatusBayar </h3>
                        </div>
                        <div>
                            <a href="{{ url('/admin/status-bayar/create') }}"
                ...
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        <tr>
                            ...
                            <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">Status Bayar</th>
                            <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">
                                <form method="GET" action="{{ url('/admin/status-bayar') }}" accept-charset="UTF-8"
                        ...
                        @if(count($statusbayar) == 0)
                            <tr>
                                <td colspan="0" class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">Belum ada data StatusBayar</td>
                            </tr>
                        @endif
                        @foreach($statusbayar as $item)
                            <tr>
                                <td class="border-t-0 pl-6 align-middle border-l-0 border-r-0 whitespace-no-wrap p-4">
                                    {{ $loop->iteration }}
                                </td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->status_bayar }}</td>
                                <td class="text-right pr-5">
                                    ...
                                    <div
                                        ...
                                        <a href="{{ url('/admin/status-bayar/' . $item->id . '/edit') }}"
                                        ...
                                        <a href="{{ url('/admin/status-bayar/' . $item->id) }}"
                                        ...
                                        <form method="POST" action="{{ url('/admin/status-bayar' . '/' . $item->id) }}"
                    ...
                    <div class="mx-6 my-4"> {!! $statusbayar->appends(['search' => Request::get('search')])->render() !!} </div>
                </div>
            </div>
        </div>
    </div>
