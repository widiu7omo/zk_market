                    ...
                    <div class="rounded-t mb-0 px-4 py-3 bg-transparent">
                        <div class="flex flex-wrap items-center">
                            ...
                            <div class="relative flex flex-wrap items-stretch lg:w-3/12 block mr-5">
                                ...
                                <form method="GET" action="{{ url('/admin/status-bayar') }}" accept-charset="UTF-8" class="m-0 p-0 w-full">
                                    <input name="search" autocomplete="off" class="px-3 py-3 placeholder-gray-600 text-gray-700 relative focus:bg-gray-100 bg-gray-300 rounded-lg text-sm outline-none focus:outline-none focus:shadow-outline w-full pl-10"
                                       placeholder="Cari data StatusBayar"
                            ...
                            <a class="uppercase tracking-wide text-sm py-3 px-3 bg-brown-lighter hover:bg-brown-dark shadow-lg rounded-lg text-white font-bold"
                               href="{{ url('/admin/status-bayar/create') }}">Tambah Status Bayar Baru</a>
                        </div>
                    </div>
                    <div class="m-4 flex-auto rounded-t-2xl rounded-2xl bg-gray-100 shadow overflow-x-scroll border-gray-700 sm:rounded-lg">
                        <table class="table-fixed rounded-2xl">
                            ...
                            <tr>
                                ...
                                <th class="px-4 py-2 w-10/12 text-gray-500 text-left tracking-wider font-light uppercase text-sm">Status Bayar</th>
                            </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($statusbayar as $item)
                            <tr>
                                <td class="px-4 py-4 text-center whitespace-no-wrap">
                                    ...
                                    <div class="hidden bg-white text-base z-50 float-left py-2 list-none text-left rounded shadow-lg mt-1"
                                        ...
                                        <a title="View StatusBayar" class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                           href="{{ url('/admin/status-bayar/' . $item->id) }}"><i class="fas fa-info-circle"></i>&nbsp; View</a>
                                        <a title="Edit StatusBayar" class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                           href="{{ url('/admin/status-bayar/' . $item->id . '/edit') }}"><i class="fas fa-pen"></i>&nbsp; Edit</a>
                                        <form method="POST" action="{{ url('/admin/status-bayar' . '/' . $item->id) }}" accept-charset="UTF-8">
                                           {{ method_field('DELETE') }}
                                           {{ csrf_field() }}
                                           <button type="submit" class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800" title="Delete StatusBayar" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fas fa-trash"></i>&nbsp; Delete</button>
                                        </form>
