                                ...
                                <tr>
                                    <td class="px-4 py-4 text-center whitespace-no-wrap">
                                        ...
                                        <div
                                            ...
                                            <a title="View StatusBayar"
                                               class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                               href="{{ url('/admin/status-bayar/' . $item->id) }}"><i
                                                    class="fas fa-info-circle"></i>&nbsp; View</a>
                                            <a title="Edit StatusBayar"
                                               class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                               href="{{ url('/admin/status-bayar/' . $item->id . '/edit') }}"><i
                                                    class="fas fa-pen"></i>&nbsp; Edit</a>
                                            <form method="POST"
                                                  action="{{ url('/admin/status-bayar' . '/' . $item->id) }}"
                                                  accept-charset="UTF-8">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit"
                                                        class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                                        title="Delete StatusBayar" onclick="return confirmModal(this)">
                                    ...
                                    <td class="whitespace-no-wrap px-4 py-4 uppercase font-bold text-sm">{{ $item->status_bayar }}</td>
                                </tr>
                            @empty
                                <tr>
