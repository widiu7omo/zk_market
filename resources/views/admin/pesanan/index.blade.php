            ...
            <div class="w-full xl:mx-auto xl:w-full mb-16 px-4 h-full">
                <div
                    ...
                    <div
                        ...
                        <table class="table-fixed rounded-2xl w-full overflow-auto">
                            ...
                            @forelse($pesanan as $item)
                                <tr>
                                    <td class="px-4 py-4 text-center whitespace-no-wrap">
                                        ...
                                        <div
                                            ...
                                            <a class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                               href="#"><i class="fa fa-sync-alt"></i>&nbsp; Proses Pembuatan</a>
                                        </div>
                                    </td>
                                    <td class=" px-4 py-4 text-center">{{ $loop->iteration }}</td>
                                    <td class="px-4 py-4">
                                        <span
                                            class="px-2 py-1 text-white text-sm shadow-lg font-bold uppercase bg-brown rounded">{{ $item->status_pesanan->status_pesanan }}</span>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="text-sm uppercase font-bold">{{ $item->alamat->customer->nama}}</div>
                                        <div class="text-sm font-italic">{{ $item->alamat->customer->no_hp}}</div>
                                    </td>
                                    <td class="px-4 py-4 text-sm">{{ $item->catatan == ''?'Tidak ada catatan':$item->catatan }}</td>
                                    <td class="px-4 py-4 text-sm font-bold text-gray-600">{{ $item->created_at->isoFormat('DD MMMM YYYY') }}
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="p-4 text-gray-800 text-center">Data Empty</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
