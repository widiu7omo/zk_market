                ...
                <div class="relative flex flex-col min-w-0 break-words h-full w-full mb-6 shadow-lg rounded-lg bg-gray-100 xl:max-w-screen-lg xl:mx-auto">
                    <div class="rounded-t mb-0 px-4 py-3 bg-transparent">
                        <div class="flex flex-wrap items-center">
                            ...
                            <form method="POST" action="{{ url('admin/kategori' . '/' . $kategori->id) }}" accept-charset="UTF-8" class="m-0 p-0">
                               {{ method_field('DELETE') }}
                               {{ csrf_field() }}
                               <button type="submit" class="uppercase tracking-wide text-sm py-3 px-3 bg-brown-lighter hover:bg-brown-dark shadow-lg rounded-lg text-white font-bold" title="Delete Kategori" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
                            </form>
                        </div>
                    </div>
                    <div class="flex flex-wrap p-3 justify-start flex-row m-3">
                        <table class="table-fixed text-gray-900">
                            ...
                            <tr><th class="w-3/12 border-r-2 font-bold tracking-wider uppercase"> Kategori </th><td class="px-4 py-2 leading-snug"> {{ $kategori->kategori }} </td></tr>
                        </table>
                    </div>
                </div>
            </div>
