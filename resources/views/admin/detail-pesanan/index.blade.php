                ...
                <div class="rounded-t mb-0 px-4 py-3 border-0">
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full pr-4 max-w-full flex-grow flex-1 flex items-center">
                            <h3 class="font-semibold text-lg text-gray-800"> Data DetailPesanan </h3>
                        </div>
                        {{--                        <div>--}}
                        {{--                            <a href="{{ url('/admin/detail-pesanan/create') }}"--}}
                ...
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        <tr>
                            ...
                            <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">
                                <form method="GET" action="{{ url('/admin/detail-pesanan') }}" accept-charset="UTF-8"
                        ...
                        @foreach($detailpesanan as $item)
                            <tr>
                                ...
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->pesanan_id }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->produk->nama }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->produk->harga }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->jumlah }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->subtotal }}</td>
                                <td class="text-right pr-5">
                                    ...
                                    <div
                                        ...
                                        <a href="{{ url('/admin/detail-pesanan/' . $item->id . '/edit') }}"
                                        ...
                                        <a href="{{ url('/admin/detail-pesanan/' . $item->id) }}"
                                        ...
                                        <form method="POST"
                                              action="{{ url('/admin/detail-pesanan' . '/' . $item->id) }}"
                    ...
                    <div
                        class="mx-6 my-4"> {!! $detailpesanan->appends(['search' => Request::get('search')])->render() !!} </div>
                </div>
            </div>
        </div>
    </div>
