                    ...
                    <div class="rounded-t mb-0 px-4 py-3 bg-transparent">
                        <div class="flex flex-wrap items-center">
                            ...
                            <div class="relative flex flex-wrap items-stretch lg:w-3/12 block mr-5">
                                ...
                                <form method="GET" action="{{ url('/admin/metode-pembayaran') }}" accept-charset="UTF-8" class="m-0 p-0 w-full">
                    ...
                    <div class="m-4 flex-auto rounded-t-2xl rounded-2xl bg-gray-100 shadow overflow-x-scroll border-gray-700 sm:rounded-lg">
                        <table class="table-fixed rounded-2xl">
                            ...
                            <tr>
                                ...
                                <th class="px-4 py-2 w-10/12 text-gray-500 text-left tracking-wider font-light uppercase text-sm">Metode</th><th class="px-4 py-2 w-10/12 text-gray-500 text-left tracking-wider font-light uppercase text-sm">Token</th><th class="px-4 py-2 w-10/12 text-gray-500 text-left tracking-wider font-light uppercase text-sm">Api</th>
                            </tr>
                            </thead>
                            ...
                            <tr>
                                <td class="px-4 py-4 text-center whitespace-no-wrap">
                                    ...
                                    <div class="hidden bg-white text-base z-50 float-left py-2 list-none text-left rounded shadow-lg mt-1"
                                        ...
                                        <a title="View MetodePembayaran" class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                           href="{{ url('/admin/metode-pembayaran/' . $item->id) }}"><i class="fas fa-info-circle"></i>&nbsp; View</a>
                                        <a title="Edit MetodePembayaran" class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800"
                                           href="{{ url('/admin/metode-pembayaran/' . $item->id . '/edit') }}"><i class="fas fa-pen"></i>&nbsp; Edit</a>
                                        <form method="POST" action="{{ url('/admin/metode-pembayaran' . '/' . $item->id) }}" accept-charset="UTF-8">
                                           {{ method_field('DELETE') }}
                                           {{ csrf_field() }}
                                           <button type="submit" class="text-sm py-2 px-4 font-normal block whitespace-no-wrap bg-transparent hover:bg-gray-300 m-2 rounded text-gray-800" title="Delete MetodePembayaran" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fas fa-trash"></i>&nbsp; Delete</button>
                                        </form>
                                ...
                                <td class="px-4 py-4">{{ $item->metode }}</td><td class="px-4 py-4">{{ $item->token }}</td><td class="px-4 py-4">{{ $item->api }}</td>
                                </tr>
                            @empty
                                <tr>
