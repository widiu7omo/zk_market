                ...
                <div class="rounded-t mb-0 px-4 py-3 border-0">
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full pr-4 max-w-full flex-grow flex-1 flex items-center">
                            <h3 class="font-semibold text-lg text-gray-800"> Data MetodePembayaran </h3>
                        </div>
                        <div>
                            <a href="{{ url('/admin/metode-pembayaran/create') }}"
                ...
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        <tr>
                            ...
                            <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">Metode</th><th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">Token</th><th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">Api</th>
                            <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">
                                <form method="GET" action="{{ url('/admin/metode-pembayaran') }}" accept-charset="UTF-8"
                                    ...
                                    <span class="z-10 h-full leading-snug font-normal absolute text-center text-gray-400 absolute bg-transparent rounded text-base items-center justify-center w-5 pl-3 py-2"><i
                        ...
                        @if(count($metodepembayaran) == 0)
                            <tr>
                                <td colspan="0" class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">Belum ada data MetodePembayaran</td>
                            </tr>
                        @endif
                        @foreach($metodepembayaran as $item)
                            <tr>
                                ...
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->metode }}</td><td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->token }}</td><td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->api }}</td>
                                <td class="text-right pr-5">
                                    ...
                                    <div
                                        ...
                                        <a href="{{ url('/admin/metode-pembayaran/' . $item->id . '/edit') }}"
                                        ...
                                        <a href="{{ url('/admin/metode-pembayaran/' . $item->id) }}"
                                        ...
                                        <form method="POST" action="{{ url('/admin/metode-pembayaran' . '/' . $item->id) }}"
                    ...
                    <div class="mx-6 my-4"> {!! $metodepembayaran->appends(['search' => Request::get('search')])->render() !!} </div>
                </div>
            </div>
        </div>
    </div>
