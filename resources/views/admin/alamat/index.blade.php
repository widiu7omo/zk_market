                ...
                <div class="rounded-t mb-0 px-4 py-3 border-0">
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full pr-4 max-w-full flex-grow flex-1 flex items-center">
                            <h3 class="font-semibold text-lg text-gray-800"> Data Alamat </h3>
                        </div>
{{--                        <div>--}}
{{--                            <a href="{{ url('/admin/alamat/create') }}"--}}
...
{{--                        </div>--}}
                ...
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        @foreach($alamat as $item)
                            <tr>
                                ...
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->alamat_lengkap }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->rincian_alamat }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->lat }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ $item->long }}</td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 w-1/3 text-md whitespace-no-wrap p-4">{{ isset($item->customer->nama)?$item->customer->nama:'NULL' }}</td>
                                <td class="text-right pr-5">
                                    ...
                                    <div
                                        ...
                                        <a href="{{ url('/admin/alamat/' . $item->id . '/edit') }}"
                                        ...
                                        <a href="{{ url('/admin/alamat/' . $item->id) }}"
                    ...
                    <div
                        class="mx-6 my-4"> {!! $alamat->appends(['search' => Request::get('search')])->render() !!} </div>
                </div>
            </div>
        </div>
    </div>
