<div class="p-2 pt-0 w-full md:w-6/12">
    <label for="waktu_pesan" class="tracking-wide uppercase  text-sm font-bold text-gray-900">{{ 'Waktu Pesan' }}
        <input required type="text" id="waktu_pesan" readonly  name="waktu_pesan" data-mask="00:00:00"
               placeholder="Input waktu_pesan"
               value="{{ isset($pesanan->waktu_pesan) ? $pesanan->waktu_pesan : ''}}"
               class="px-3 py-3 mt-2 placeholder-gray-400 bg-gray-200 {{ $errors->has('waktu_pesan') ? 'text-red-700' : 'text-gray-700'}} relative bg-white rounded text-sm shadow focus:outline-none border-2 {{ $errors->has('waktu_pesan') ? 'border-red-600' : 'border-white'}} focus:border-transparent w-full transition duration-200 ease-in-out"/>
...
<div class="p-2 pt-0 w-full md:w-6/12">
    <label for="tanggal" class="tracking-wide uppercase  text-sm font-bold text-gray-900">{{ 'Tanggal' }}
        <input required type="text" id="tanggal" readonly  name="tanggal" placeholder="Input tanggal"
               value="{{ isset($pesanan->tanggal) ? $pesanan->tanggal : ''}}"
               class="px-3 py-3 mt-2 placeholder-gray-400 bg-gray-200  {{ $errors->has('tanggal') ? 'text-red-700' : 'text-gray-700'}} relative bg-white rounded text-sm shadow focus:outline-none border-2 {{ $errors->has('tanggal') ? 'border-red-600' : 'border-white'}} focus:border-transparent w-full transition duration-200 ease-in-out"/>
